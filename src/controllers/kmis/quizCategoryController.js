...
exports.index = async (req, res) => {
  ...
  try {
    ...
    const response = new WithDataResource(
      ...
      "Data kategori kategori soal pertanyaan berhasil diambil.",
      {
        data: serializedData,
    ...
    logger.error(
      `| Quiz Category KMIS | - Error function index : ${error.message}`
    );
    const response = new WithoutDataResource(
      500,
      "SERVER_ERROR",
...
exports.store = async (req, res) => {
  ...
  const { categoryId, topicId, name, description, totalQuestion } = req.body;
  ...
  try {
    ...
    const exists = await trx("kmis_quiz_categories")
    ...
    if (exists) {
      const response = new WithoutDataResource(
        ...
        "DUPLICATE_TITLE",
        "Duplikat Data",
        "Ada nama kategori kategori soal pertanyaan yang sama dengan yang anda buat. Silakan buat nama yang lain."
      );
      return res.status(400).json(response.toResponse());
    }

    await trx("kmis_quiz_categories")
      .insert({
    ...
    await activityLogHelper.logCreate(
      {
        ...
        module: "kmis",
        subject: "List Kategori Soal Pertanyaan",
      },
      trx
    );
    ...
    const response = new WithoutDataResource(
      ...
      "Data kategori soal pertanyaan berhasil ditambahkan."
    );
    return res.status(201).json(response.toResponse());
  } catch (error) {
    await trx.rollback();
    logger.error(
      `| Quiz Category KMIS | - Error function store: ${error.message}`
    );
    const response = new WithoutDataResource(
      500,
      "SERVER_ERROR",
  ...
  try {
    ...
    if (!quiz) {
      const response = new WithoutDataResource(
        ...
        "DATA_NOT_FOUND",
        "Data Tidak Ditemukan",
        `Data kategori soal pertanyaan dengan ID '${id}' tidak ditemukan.`
      );
      return res.status(200).json(response.toResponse());
    }
    ...
    const response = new WithDataResource(
      ...
      "SUCCESS_GET_DATA",
      "Berhasil Mengambil Data",
      "Detail data kategori soal pertanyaan berhasil didapatkan.",
      data
    );
    return res.status(200).json(response.toResponse());
  } catch (error) {
    logger.error(
      `| Quiz Category KMIS | - Error function show: ${error.message}`
    );
    const response = new WithoutDataResource(
      500,
      "SERVER_ERROR",
...
exports.update = async (req, res) => {
  ...
  try {
    ...
    const existing = await trx("kmis_quiz_categories").where("id", id).first();
    if (!existing) {
      const response = new WithoutDataResource(
        200,
        "DATA_NOT_FOUND",
    ...
    const duplicate = await trx("kmis_quiz_categories")
      .whereRaw("lower(name) = lower(?)", [name])
      .whereNull("deleted_at")
      .whereNot("id", id)
      .first();
    if (duplicate) {
      const response = new WithoutDataResource(
        ...
        "DUPLICATE_TITLE",
        "Duplikat Data",
        "Ada kategori soal pertanyaan yang sama dengan yang anda perbarui. Silakan buat soal yang lain."
      );
      return res.status(400).json(response.toResponse());
    }
    ...
    await trx("kmis_quiz_categories")
      ...
      .update({
        ...
        total_question: totalQuestion ?? existing.total_question,
    ...
    const response = new WithoutDataResource(
      ...
      "Data kategori soal pertanyaan berhasil diperbarui."
    );
    return res.status(200).json(response.toResponse());
  } catch (error) {
    await trx.rollback();
    logger.error(
      `| Quiz Category KMIS | - Error function update : ${error.message}`
    );
    const response = new WithoutDataResource(
      500,
      "SERVER_ERROR",
...
exports.destroy = async (req, res) => {
  ...
  try {
    ...
    const existing = await trx("kmis_quiz_categories")
      .select("id", "name")
      .whereIn("id", ids)
      .whereNull("deleted_at");
    if (existing.length === 0) {
      ...
      const response = new WithoutDataResource(
        ...
        "DATA_NOT_FOUND",
        "Data Tidak Ditemukan",
        `Tidak ada data kategori soal pertanyaan yang cocok atau sudah terhapus.`
      );
      return res.status(200).json(response.toResponse());
    }
    ...
    await trx("kmis_quiz_categories").whereIn("id", existingIds).update({
    ...
    const response = new WithoutDataResource(
      ...
      `Berhasil menghapus (soft delete) ${existingIds.length} data kategori soal pertanyaan.`
    );
    return res.status(200).json(response.toResponse());
  } catch (error) {
    await trx.rollback();
    logger.error(
      `| Quiz Category KMIS | - Error function destroy : ${error.message}`
    );
    const response = new WithoutDataResource(
      500,
      "SERVER_ERROR",
...
exports.restore = async (req, res) => {
  ...
  try {
    ...
    const softDeleted = await trx("kmis_quiz_categories")
      .select("id", "name")
      .whereIn("id", ids)
    ...
    if (softDeleted.length === 0) {
      ...
      const response = new WithoutDataResource(
        ...
        "DATA_NOT_FOUND",
        "Data Tidak Ditemukan",
        `Tidak ada data kategori soal pertanyaan terhapus yang cocok untuk direstore.`
      );
      return res.status(200).json(response.toResponse());
    }
    ...
    const activeWithSameTitle = await trx("kmis_quiz_categories")
    ...
    logger.error(
      `| Quiz Category KMIS | - Error function restore: ${error.message}`
    );
    const response = new WithoutDataResource(
      500,
      "SERVER_ERROR",
