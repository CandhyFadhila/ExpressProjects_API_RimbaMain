...
exports.index = async (req, res) => {
  ...
  try {
    let query = knex("monev_activity_categories as category").select("category.*");
    ...
    const serializedData = await Promise.all(
      result.data.map((category) => activityCategoryResource(category))
    );
    ...
    logger.error(`| Activity Category MONEV | - Error function index : ${error.message}`);
    const response = new WithoutDataResource(
      500,
      "SERVER_ERROR",
      "Server Sedang Error",
...
exports.store = async (req, res) => {
  ...
  try {
    ...
    const exists = await trx("monev_activity_categories")
    ...
    await trx("monev_activity_categories")
      .insert({
        title,
        description,
      })
    ...
    await activityLogHelper.logCreate(
      {
        ...
        module: "master_data",
        subject: "List Kategori Aktivitas Kegiatan",
      },
      trx
    );
    ...
    await trx.rollback();
    logger.error(`| Activity Category MONEV | - Error function store: ${error.message}`);
    const response = new WithoutDataResource(
      500,
      "SERVER_ERROR",
      "Server Sedang Error",
...
exports.show = async (req, res) => {
  ...
  try {
    ...
    const data = await activityCategoryResource(category);
    const response = new WithDataResource(
      200,
      "SUCCESS_GET_DATA",
      "Berhasil Mengambil Data",
    ...
    logger.error(`| Activity Category MONEV | - Error function show: ${error.message}`);
    const response = new WithoutDataResource(
      500,
      "SERVER_ERROR",
      "Server Sedang Error",
...
exports.update = async (req, res) => {
  ...
  try {
    ...
    const existing = await trx("monev_activity_categories").where("id", id).first();
    if (!existing) {
      const response = new WithoutDataResource(
        200,
        "DATA_NOT_FOUND",
    ...
    const duplicate = await trx("monev_activity_categories")
      .whereRaw("lower(title) = lower(?)", [title])
      .whereNull("deleted_at")
      .whereNot("id", id)
      .first();
    ...
    await trx("monev_activity_categories").where("id", id).update({
      title,
      description,
    ...
    await trx.rollback();
    logger.error(
      `| Activity Category MONEV | - Error function update : ${error.message}`
    );
    const response = new WithoutDataResource(
      500,
      "SERVER_ERROR",
...
exports.destroy = async (req, res) => {
  ...
  try {
    ...
    const existing = await trx("monev_activity_categories")
      .select("id", "title")
      .whereIn("id", ids)
      .whereNull("deleted_at");
    ...
    await trx("monev_activity_categories").whereIn("id", existingIds).update({
    ...
    await trx.rollback();
    logger.error(
      `| Activity Category MONEV | - Error function destroy : ${error.message}`
    );
    const response = new WithoutDataResource(
      500,
      "SERVER_ERROR",
...
exports.restore = async (req, res) => {
  ...
  try {
    ...
    const softDeleted = await trx("monev_activity_categories")
      .select("id", "title")
      .whereIn("id", ids)
    ...
    const activeWithSameTitle = await trx("monev_activity_categories")
    ...
    logger.error(
      `| Activity Category MONEV | - Error function restore: ${error.message}`
    );
    const response = new WithoutDataResource(
      500,
      "SERVER_ERROR",
